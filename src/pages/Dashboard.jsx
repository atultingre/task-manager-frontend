...
import { Chart, UserInfo } from "../Components";
...
const TaskTable = ({ tasks }) => {
  ...
  const TableHeader = () => (
    <thead className="border-b text-sm border-gray-300">
  ...
  const TableRow = ({ task }) => (
    <tr className="border-b text-sm border-gray-300 text-gray-600 hover:bg-gray-300/10">
      <td className="py-2">
        <div className="flex items-center gap-2">
          ...
          <p className=" text-black text-sm">{task.title}</p>
        </div>
      </td>
  ...
  return (
    <>
      <div className="w-full lg:w-2/3 bg-white px-4 md:px-4 pt-4 pb-4 shadow-md rounded">
...
const UserTable = ({ users }) => {
  ...
  const TableRow = ({ user }) => (
    <tr className="border-b border-gray-200 text-sm text-gray-600 hover:bg-gray-400/10">
      <td className="py-2">
      ...
      <td>
        <p
          className={clsx(
            "w-fit px-1.5 py-0.5 rounded-full text-sm",
      ...
      <td className="py-2 text-xs">{moment(user?.createdAt).fromNow()}</td>
    </tr>
  );
